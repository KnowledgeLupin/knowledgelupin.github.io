...
> 원문: [Functional Programming in Lean](https://lean-lang.org/functional_programming_in_lean/Introduction/)  
...
### 핵심 개념 요약
- Lean의 이중 정체성(정리 증명기이자 프로그래밍 언어)과 학습 목표를 소개합니다.
- strict 평가, 순수성, 종속 타입이 Lean 프로그래밍 경험을 어떻게 바꾸는지 정리합니다.
- 도구 체인과 학습 순서를 제시해 이후 장을 읽기 위한 기반을 마련합니다.

### 학습 포인트
1. 환경 설치(`elan`, `lake`, 에디터 플러그인)를 먼저 안정화한 뒤 예제를 따라가면 학습 효율이 높습니다.
2. 용어를 처음부터 고정해 두면 이후 장의 타입 클래스/모나드/종속 타입 내용을 더 빠르게 연결할 수 있습니다.
