...
## 동기: 왜 공리부터 시작할까?
선형대수에서 우리가 가장 자주 마주치는 문제는 $A\mathbf{x}=\mathbf{b}$ 같은 연립선형문제다.  
하지만 "벡터를 더한다", "스칼라를 곱한다"는 연산의 규칙이 분명하지 않으면 같은 계산도 맥락마다 다른 결과를 낼 수 있고, 증명은 성립하지 않는다.  
그래서 먼저 스칼라의 체(field) 구조와 벡터공간의 공리를 고정해, 계산과 논증이 항상 같은 문법 위에서 작동하도록 만든다.  
이 절의 목표는 정의를 나열하는 데 있지 않으며, 이후의 기저, 차원, 선형사상, 고유값 이론의 논리적 기반을 분명히 하는 데 있다.
